import clientModel from "../../models/jobsModel.js"; // adjust path to your actual Client model
...
export const getNewSales = async (req, res) => {
  try {
    const { start, end } = parseDateRange(req.query);
    const { groupBy } = req.query;

    const matchQuery = applyFilters({...progressFilters}, req.query, [
    //   ["source", "source"],
...
export const getSubscriptionSales = async (req, res) => {
  try {
    const { start, end } = parseDateRange(req.query);
    const { groupBy } = req.query;

    const matchQuery = applyFilters({progressStatus: "completed"}, req.query, [
      ["source", "source"],
      ["jobHolder", "job.jobHolder"],
    //   ["partner", "partner"],
