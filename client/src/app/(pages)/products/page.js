  ...
  const fetchProjects = async () => {
    ...
    try {
      const { data } = await axios.get(
        `${process.env.NEXT_PUBLIC_SERVER_URI}/api/v1/product/getAll`
      );
    ...
    () => [
      ...
      },
      {
        ...
        Header: ({ column }) => {
          return (
            <div className=" flex flex-col gap-[2px]">
              <span className="ml-1 cursor-pointer truncate">Brand</span>
            </div>
          );
        },
        Cell: ({ cell, row }) => {
      ...
      },
      {
        ...
        Header: ({ column }) => {
          return (
            <div className=" flex flex-col gap-[2px]">
              <span className="ml-1 cursor-pointer truncate">QR Code</span>
            </div>
          );
        },
        Cell: ({ cell, row }) => {
          const qr_code = row.original?.qr_code;
