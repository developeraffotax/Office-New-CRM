// Place next to Filters.jsx (e.g. whatsapp/shared/FiltersMobile.jsx)
// Same props as your desktop WhatsApp Filters.
// Structure mirrors the Inbox Gmail FiltersMobile (bottom sheet pattern).
...
import ManageCategoriesModal from "./ui/ManageCategoriesModal"; // adjust path if needed
...
export default function FiltersMobile({
  ...
  team = [],
  categories = [],
  fetchConversations, // optional – same role as fetchThreads in Inbox
}) {
  const {
    auth: { user },
  ...
  const hasUnassignedPermission = useMemo(
    () => hasSubrole(user, "Whatsapp", "Unassigned"),
    [user],
  );
  ...
  // Fetch user counts (WhatsApp endpoint)
  useEffect(() => {
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      ...
      >
        ...
        <Stack
          ...
          spacing={1}
          alignItems="center"
          sx={{ p: 1.5, marginLeft: 6 }} // marginLeft accounts for hamburger
        >
          {hasActiveFilters && (
            <IconButton
