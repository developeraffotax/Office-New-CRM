...
import WonLeadStats from "./ProposalStats";
...
export default function UserProposalChart({ auth, active1 }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const { data } = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/v1/proposal/userchart/won`,
        {
          params: {
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      ...
      >
        <div className="mb-2 w-full  h-[100px]   flex justify-between items-end  gap-5 ">
          ...
          <ProposalStats user={user} dateRange={dateRange} />
        </div>
...
<h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2 mb-4">
  ...
  <span>
    User Proposal Statistics <span className="text-gray-500 font-normal"></span> – {user}
  </span>
</h2>
            <div>
              <select
