...
import { getOnlineAgentViaHeartbeat } from "../../utils/onlineStatus.js";
...
export const checkRunningTimers = async () => {
  try {
    ...
    // ── 2. Check heartbeat for each timer owner in parallel ────────────────
    const heartbeatResults = await Promise.all(
      runningTimers.map(async (t) => ({
        clientId: t.clientId.toString(),
        alive: await getOnlineAgentViaHeartbeat(t.clientId),
      }))
    );

    const heartbeatSet = new Set(
      heartbeatResults.filter((r) => r.alive).map((r) => r.clientId)
    );

    console.log(`💓 [CRON] Online via heartbeat: ${[...heartbeatSet].join(", ") || "none"}`);
    console.log(`⏱  [CRON] Running timers      : ${runningTimers.length}`);

    // ── 3. Stop timers where heartbeat is missing ──────────────────────────
    const timersToStop = runningTimers.filter(
      (t) => !heartbeatSet.has(t.clientId.toString())
    );
    ...
    await timerModel.updateMany(
      { _id: { $in: timerIds }, isRunning: true },
      {
        $set: {
          isRunning: false,
