...
// import { openReplyModal,closeEmailDrawer, closeReplyModal, } from "../../redux/slices/emailDrawerSlice";
 

// export default function EmailDetailDrawerGlobal() {

//   const dispatch = useDispatch();

// const { isDrawerOpen, ticketId, isReplyOpen } = useSelector(
//   (state) => state.emailDrawer
// );
...
//       `${process.env.REACT_APP_API_URL}/api/v1/tickets/activity/map/${ticketId}`
//     );
...
//     try {
//       const { data } = await axios.get(
//         `${process.env.REACT_APP_API_URL}/api/v1/tickets/single/ticket/${ticketId}`
//       );
//       if (data?.ticket) {
...
//         // setEmailData((prevData) => prevData.map((item) => item._ticketId === data.ticket._ticketId ? data.ticket : item));
...
//     try {
//       const { data } = await axios.get(
//         `${process.env.REACT_APP_API_URL}/api/v1/tickets/single/email/detail/${mailThreadId}/${company}/${ticketId}`
//       );
...
//       const { data } = await axios.get(
//         `${process.env.REACT_APP_API_URL}/api/v1/tickets/single/email/detail/${ticketDetail.mailThreadId}/${ticketDetail.company}/${ticketId}`
//       );
...
//     }
//     try {
//       const { data: {success, updatedTicket, message}  } = await axios.put(`${process.env.REACT_APP_API_URL}/api/v1/tickets/markAsRead/${ticketId}`, {
//         messageId,
...
//           <button
//             onClick={() => handleUpdateTicketStatusConfirmation(ticketId)}
...
//             onClick={() => dispatch(openReplyModal())}
//             style={{ padding: ".4rem 1rem" }}
//             disabled={loading}
//           >
...
//       {/* Email Reply */}
//       {isReplyOpen && (
//   <div className="fixed top-0 left-0 z-[999] w-full h-full bg-gray-700/70 flex items-center justify-center">
//     <SendEmailReply
//       setShowReply={() => dispatch(closeReplyModal())}
